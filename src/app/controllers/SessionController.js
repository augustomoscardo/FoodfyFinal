  ...
  async forgot(req, res) {
    ...
    try {
      ...
      await mailer.sendMail({
        ...
        html: `<h2 style="font-size: 24px; font-weight: bold;">Esqueceu sua senha, ${user.name}?</h2>   
        <p> Não se preocupe, clique no link abaixo para recuperar sua senha.</p>
        <p style="text-align: center;">
          <a 
            style="display: block; margin: 0 auto; padding: 16px; width: 150px; color: #fff;
            background-color: #6558C3; text-decoration: none; border-radius: 4px; text-transform: uppercase;
            font-weight:bold"
            href="http://localhost:3000/admin/users/reset-password?token=${token}" target="_blank"
          >
            RECUPERAR SENHA
          </a>
        </p>
        <p style="padding-top:16px; border-top: 2px solid #ccc">Acesse sua conta já! Estamos te aguardando.</p>
        <p>Equipe Foodfy.</p>
        `
      })
